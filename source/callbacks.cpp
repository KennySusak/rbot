...
{
    if (g_needFakePingResend && engine->GetTime() >= g_fakePingResendTime)
    {
        g_needFakePingResend = false;

        // re-send scoreboard pings for all humans
        ReSendFakePingAll();
    }
}

void BotCallbacks::levelInitialize(void)
{
}
